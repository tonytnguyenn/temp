...
void compute_temp() {
    ...
    for (int i=0;i<num_iterations;i++) {
        ...
        // Applying OpenMP parallelism 
        #pragma omp parallel
        {
            // Compute the ceiling of the number of active rows divided by the number of threads
            int num_threads = omp_get_num_threads();
            int active_rows = active_rows_on_proc; // Active rows handled by this MPI process

            // Update the Temp_buf array for all active rows
            #pragma omp for
            ...
            // Implicit barrier at the end of omp for - all threads wait until all rows are computed

            // Update the Temp array for all active rows
            #pragma omp for
            for (int j = 1; j <= active_rows; j++) {
                for (int k = 1; k <= total_cols - 2; k++) {
    ...
    if (argc!=4 && argc!=5) {
        if (rank==0) {
            std::cout << "Usage: mpirun -np [No. procs] " << argv[0] <<
                " [OpenMP threads] [Dim n] [No. Iterations] [Suppress Output]"
                << std::endl;
        }
        MPI_Finalize();
        exit(0);
